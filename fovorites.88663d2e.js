async function t(t){try{let e=await fetch(t);return await e.json()}catch(t){console.log("err",t)}}const e=document.querySelector(".exercises__list"),s=document.querySelector(".exercises-cards__pagination");function a(){let a=null,i=s=>{t(s).then(t=>{e&&(e.innerHTML="",t.results.forEach(t=>{e.insertAdjacentHTML("afterbegin",function({name:t,filter:e}){return`<li class="exercises__item">
    <div class="exercises__wrap">
        ...
        <p class="exercises-card__category">${e}</p>
            ...
            ), url(${t.imgUrl})`,s.style.backgroundSize="cover")}))})},o=t=>{if(s){s.innerHTML="";for(let e=0;e<t;e++)s.insertAdjacentHTML("beforeend",`<li class="exercises-pagination__item">
            <button class="exercises-cards__page">${e+1}</button>
          </li>`);(a=s.querySelector("button"))&&a.classList.add("exercises-active-page")}};t("https://energyflow.b.goit.study/api/filters?filter=Muscles&page=1&limit=12").then(t=>{o(t.totalPages),i("https://energyflow.b.goit.study/api/filters?filter=Muscles&page=1&limit=12"),s&&s.addEventListener("click",t=>{if("BUTTON"!==t.target.nodeName)return;a&&a.classList.remove("exercises-active-page"),(a=t.target).classList.add("exercises-active-page");let e=t.target.textContent;i(`https://energyflow.b.goit.study/api/filters?filter=Muscles&page=${e}&limit=12`)})})}function i({rating:t,name:e,burnedCalories:s,time:a,bodyPart:i,target:o,_id:r}){return e.length>29&&window.screen.width>=1440?`<li class="body-parts__item">
    ...
    <div class="body-parts-about-box">
      ...
      <p class="body-parts-about__title">
        Target: <span class="body-parts-about__span">${o[0].toUpperCase()+o.slice(1)}</span>
      </p>
    </div>
    ...
    <div class="body-parts-about-box">
      ...
      <div class="body-parts-about-box">
    ...
    </li>`}async function o(t){try{let e=await fetch(`https://energyflow.b.goit.study/api/exercises/${t}`),s=await e.json();return console.log(s),s}catch(t){console.log("err",t)}}const r=document.querySelector(".favorites-exercises__list");function l(){if(localStorage.getItem("favorites")){let t=JSON.parse(localStorage.getItem("favorites"));t.length?function(t){let e=t.map(t=>i(t));r&&(r.innerHTML=e.join("")),document.querySelectorAll(".body-parts-start__text").forEach(t=>t.addEventListener("click",d))}(t):r.innerHTML=`<div class="favorites-info">
        ...
        </div>`}else r.innerHTML=`<div class="favorites-info">
            <img class="favorites-info-img" src="./images/favorites/dumbbell.png" alt="dumbbell">
            <p class="favorites-info-text">It appears that you haven't added any exercises to your favorites yet. To get started, you can add exercises that you like to your favorites for easier access in the future\u{1F60A}</p>
        </div>`}l();const n=document.querySelector("body");async function d(t){let e=!1,s=t.currentTarget.dataset.id,a=await o(s);function i(t){let e=document.querySelector(".favorite-wrap");t?(e.innerHTML=`<button type="button" class="modal-remove-favorite-btn modal-btn">Remove from 
...
</svg></button>`,document.querySelector(".modal-remove-favorite-btn").addEventListener("click",l)):(e.innerHTML=`<button type="button" class="modal-add-favorite-btn modal-btn">Add to favorite<svg class="modal-btn-heart" width="20" height="20" viewBox="0 0 20 20" fill="none" xmlns="http://www.w3.org/2000/svg">
      <path d="M17.3671 3.84166C16.9415 3.41583 16.4361 3.07803 15.8799 2.84757C15.3237 2.6171 14.7275 2.49847 14.1254 2.49847C13.5234 2.49847 12.9272 2.6171 12.371 2.84757C11.8147 3.07803 11.3094 3.41583 10.8838 3.84166L10.0004 4.725L9.11709 3.84166C8.25735 2.98192 7.09128 2.49892 5.87542 2.49892C4.65956 2.49892 3.4935 2.98192 2.63376 3.84166C1.77401 4.70141 1.29102 5.86747 1.29102 7.08333C1.29102 8.29919 1.77401 9.46525 2.63376 10.325L3.51709 11.2083L10.0004 17.6917L16.4838 11.2083L17.3671 10.325C17.7929 9.89937 18.1307 9.39401 18.3612 8.83779C18.5917 8.28158 18.7103 7.6854 18.7103 7.08333C18.7103 6.48126 18.5917 5.88508 18.3612 5.32887C18.1307 4.77265 17.7929 4.26729 17.3671 3.84166Z" stroke="#F6F6F6" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round"/>
      </svg></button>`,document.querySelector(".modal-add-favorite-btn").addEventListener("click",r))}function r(){let t=JSON.parse(localStorage.getItem("favorites"));e=!0,t.push(a),localStorage.setItem("favorites",JSON.stringify(t)),i(e)}function l(){let t=JSON.parse(localStorage.getItem("favorites"));e=!1;let s=t.filter(t=>t._id!==a._id);localStorage.setItem("favorites",JSON.stringify(s)),i(e)}localStorage.getItem("favorites")&&JSON.parse(localStorage.getItem("favorites")).map(t=>t._id).includes(a._id)&&(e=!0),function({name:t,equipment:e,burnedCalories:s,bodyPart:a,popularity:i,_id:o,time:r,target:l,rating:d,gifUrl:p},c){let g=`
    <div class="backdrop">
<div class="modal">
...
<div class="modal-img-wrap">
    <img class="modal-img" src="${p}" alt="exercise">
</div>
<div class="modal-content-wrap">
<div >
    <h2 class="modal-title">${t}</h2>
    <div class="modal-rating">
        <p class="modal-rating-amount">${Math.round(d)}.0</p>
        <ul class="modal-rating-list">
            ${[1,2,3,4,5].map((t,e)=>e<Math.round(d)?`<li class="modal-rating-item"><svg class="modal-rating-stars gold-star"><svg width="14" height="13" viewBox="0 0 14 13" fill="none" xmlns="http://www.w3.org/2000/svg">
                  <path d="M6.04894 0.927052C6.3483 0.0057416 7.6517 0.00574088 7.95106 0.927052L8.79611 3.52786C8.92999 3.93989 9.31394 4.21885 9.74717 4.21885H12.4818C13.4505 4.21885 13.8533 5.45846 13.0696 6.02786L10.8572 7.63525C10.5067 7.8899 10.3601 8.34127 10.494 8.75329L11.339 11.3541C11.6384 12.2754 10.5839 13.0415 9.80017 12.4721L7.58779 10.8647C7.2373 10.6101 6.7627 10.6101 6.41222 10.8647L4.19983 12.4721C3.41612 13.0415 2.36164 12.2754 2.66099 11.3541L3.50604 8.75329C3.63992 8.34127 3.49326 7.8899 3.14277 7.63525L0.930391 6.02787C0.146677 5.45846 0.549452 4.21885 1.51818 4.21885H4.25283C4.68606 4.21885 5.07001 3.93989 5.20389 3.52786L6.04894 0.927052Z" fill="#EEA10C"/>
    ...
    <ul class="modal-details-list">
        ...
        <li class="modal-details-item">
            ...
            <p class="modal-details-item-text">${s}/${r} min</p>
        </li>
    </ul>
...
<button type="button" class="modal-add-rating-btn modal-btn">Give a rating</button>
    ...
    `;n.insertAdjacentHTML("afterbegin",g)}(a,0),document.querySelector(".close-btn").addEventListener("click",p),i(e),document.querySelector(".modal-add-rating-btn")}function p(){document.querySelector(".backdrop").remove(),document.location.href.includes("fovorites")&&l()}const c=document.querySelector(".exercises__list"),g=document.querySelector(".exercises-cards__pagination"),u=document.querySelector(".exercises__list"),b=document.querySelector(".exercises-btns__list"),v=document.querySelector(".exercises-cards__pagination");a(),u&&b.addEventListener("click",e=>{"UL"!==e.target.nodeName&&"LI"!==e.target.nodeName&&(u.innerHTML="",v.innerHTML="",e.target.hasAttribute("data-muscles")&&a(),e.target.hasAttribute("data-body-parts")&&function(){let e=null,s=e=>{t(e).then(t=>{c.innerHTML="",t.results.forEach(t=>{c.insertAdjacentHTML("beforeend",i(t))}),document.querySelectorAll(".body-parts-start__text").forEach(t=>t.addEventListener("click",d))})},a=t=>{g.innerHTML="";for(let e=0;e<t;e++)g.insertAdjacentHTML("beforeend",`<li class="exercises-pagination__item">
          <button class="exercises-cards__page">${e+1}</button>
        </li>`);(e=g.querySelector("button"))&&e.classList.add("exercises-active-page")};t("https://energyflow.b.goit.study/api/exercises?page=1&bodypart=waist&limit=9").then(t=>{a(t.totalPages),s("https://energyflow.b.goit.study/api/exercises?page=1&bodypart=waist&limit=9"),g.addEventListener("click",t=>{if("BUTTON"!==t.target.nodeName)return;e&&e.classList.remove("exercises-active-page"),(e=t.target).classList.add("exercises-active-page");let a=t.target.textContent;s(`https://energyflow.b.goit.study/api/exercises?page=${a}&bodypart=waist&limit=9`)})})}())});const m=document.querySelector(".footer__form");m&&m.addEventListener("submit",t=>{t.preventDefault();let e={email:t.currentTarget.elements.email.value};console.log(e),fetch("https://energyflow.b.goit.study/api/subscription",{method:"POST",body:JSON.stringify(e),headers:{"Content-Type":"application/json; charset=UTF-8"}}).then(t=>t.json()).then(t=>console.log(t)).catch(t=>console.log(t)),t.currentTarget.reset()});
//# sourceMappingURL=fovorites.88663d2e.js.map
