...
<path d="M17.3671 3.84166C16.9415 3.41583 16.4361 3.07803 15.8799 2.84757C15.3237 2.6171 14.7275 2.49847 14.1254 2.49847C13.5234 2.49847 12.9272 2.6171 12.371 2.84757C11.8147 3.07803 11.3094 3.41583 10.8838 3.84166L10.0004 4.725L9.11709 3.84166C8.25735 2.98192 7.09128 2.49892 5.87542 2.49892C4.65956 2.49892 3.4935 2.98192 2.63376 3.84166C1.77401 4.70141 1.29102 5.86747 1.29102 7.08333C1.29102 8.29919 1.77401 9.46525 2.63376 10.325L3.51709 11.2083L10.0004 17.6917L16.4838 11.2083L17.3671 10.325C17.7929 9.89937 18.1307 9.39401 18.3612 8.83779C18.5917 8.28158 18.7103 7.6854 18.7103 7.08333C18.7103 6.48126 18.5917 5.88508 18.3612 5.32887C18.1307 4.77265 17.7929 4.26729 17.3671 3.84166Z" stroke="#F6F6F6" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round"/>
  ...
  const giveARatingBtnRef = document.querySelector('.modal-add-rating-btn');
  const sendRatingBtnRef = document.querySelector('.rating-modal-send__btn');
  const closeRatingModalBtnRef = document.querySelector(
    '.rating-modal-close__btn'
  );
  const ratingFormRef = document.querySelector('.rating-modal__form');
  const ratingModalTextRef = document.querySelector('.rating-modal__text');
  const ratingModalStarSvgRef = document.querySelector(
    '.rating-modal-star__svg'
  );

  giveARatingBtnRef.addEventListener('click', evt => {
  ...
  ratingFormRef.addEventListener('submit', evt => {
    ...
    if (
      ...
      evt.target.elements.ratingEmail.value.includes('.') === false ||
      evt.target.elements.ratingEmail.value[
        evt.target.elements.ratingEmail.value.length - 1
      ] === '.'
    ) {
    ...
    sendReview().then(data => {
      ...
      success({
        title: 'Success!',
        text: 'Your rating has been sent successfully!',
      });
    });
    document
...
<button type="button" class="modal-add-rating-btn modal-btn" data-id="${_id}">Give a rating</button>
    </div >
</div >
</div >
</div >
